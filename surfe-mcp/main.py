...
@mcp.tool()
async def cerca_persone(
    titolo: str,
    settore: str = "",
    paese: str = "IT",
    max_risultati: int = 25
) -> dict:
    """Cerca persone su Surfe per titolo, settore e paese. Restituisce nome, azienda, LinkedIn."""
    payload = {
        "filters": {
            "jobTitles": [titolo],
            "countries": [paese],
...
@mcp.tool()
async def arricchisci_persona(
    linkedin_url: str,
    vuoi_email: bool = True,
    vuoi_mobile: bool = False
) -> dict:
    """Dato un URL LinkedIn, restituisce email e/o numero di telefono della persona."""
    payload = {
        "include": {"email": vuoi_email, "mobile": vuoi_mobile},
        "people": [{"linkedinUrl": linkedin_url}]
    }
    async with httpx.AsyncClient() as client:
...
async def cerca_aziende(
    settore: str,
    paese: str = "IT",
    dipendenti_min: int = 0,
    dipendenti_max: int = 10000,
    max_risultati: int = 25
) -> dict:
    ...
    payload = {
        "filters": {
            "countries": [paese],
            "industries": [settore],
            "numberOfEmployees": {"min": dipendenti_min, "max": dipendenti_max}
        },
        "limit": max_risultati
    }
    async with httpx.AsyncClient() as client:
...
async def arricchisci_azienda(dominio: str) -> dict:
    """Dato il dominio di un'azienda (es: dedaloceramiche.it), restituisce dati completi."""
